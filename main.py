...
class ObjModel:
    def __init__(self, path: str):
        self.vertexes: list[tuple[float, float, float]] = []
    ...
    def draw_vertexes(self, img: CustomImage, color: Color, transform: Transformation):
    ...
    def draw_faces(self, img: CustomImage, color: Color, transform: Transformation):
        for v1, v2, v3 in self.faces:
            x, y, z = self.vertexes[v1]
            x0, y0 = transform(x, y, z)
            x, y, z = self.vertexes[v2]
            x1, y1 = transform(x, y, z)
            x, y, z = self.vertexes[v3]
